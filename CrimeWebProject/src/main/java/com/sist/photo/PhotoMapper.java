...
public interface PhotoMapper {
	...
	/*
	 * SELECT b_no, b_subject, m_nick, b_regdate, b_grade, b_hit, num FROM
	 * (SELECT b_no, b_subject, m_nick,b_regdate, b_grade, b_hit, rownum as num
	 * FROM (SELECT b.b_no, b.b_subject, m.m_nick, b.b_regdate, b.b_grade,
	 * b.b_hit FROM board b, member m WHERE b_grade=#{grade} AND
	 * b.m_email=m.m_email ORDER BY b.b_no DESC)) WHERE num BETWEEN #{start} AND
	 * #{end}
	 */
	@Select("SELECT no,gu,pwd,regdate,subject,content,filename,filecount,good,addr, num"
			...
			+ "	FROM photoboard WHERE gu=#{gu} order By no desc))" + "	WHERE num BETWEEN #{start} AND #{end}")
	public List<PhotoVO> p_ContentList(Map map);

	@Select("SELECT rank,good,no, gu, pwd,regdate,subject,content,filename,filecount,addr "
			+ "FROM (SELECT DENSE_RANK() OVER (ORDER BY good desc) as rank , good,no, gu, pwd,regdate,subject,content,filename,filecount,addr "
			+ "FROM photoboard WHERE gu=#{gu}) " + "WHERE rownum <= 4")
